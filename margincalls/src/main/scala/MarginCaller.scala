...
object MarginCaller extends App {
  ...
  for (i <- 0 to equities.length - 1) {
    ...
    val todaysPriceDF = pricesDFBuffer(i)
      .selectExpr("EQUITY", "PRICE AS PRICE_TODAY")
      .where(col("DATE") === (lit(today)))

    val yesterdaysPriceDF = pricesDFBuffer(i).
       selectExpr("EQUITY", "PRICE AS PRICE_YESTERDAY")
  ...
  val dealsValuationsDF = dealsDF
    .join(instrumentsValuationsDF, instrumentsValuationsJoin)
