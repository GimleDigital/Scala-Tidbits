...
/** A simple example app that cleans input data for some equity option trades
 *  and store them in files to be used in another program, SampleCreator.
 *
 *  Files are generated for some Eurex listed options on equities quoted at
 *  the Helsinki stock exchange on a historical date.
 *
